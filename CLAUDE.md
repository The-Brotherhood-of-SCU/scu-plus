...
- **Version check**: The popup checks for updates by fetching the remote `package.json` from gitee.com (configured in `package.json` → `checkForUpdatePkgLink`) via the background proxy. Version comparison is in `script/utils.ts` → `checkVersion()`.
