...
function initialize(): void {
  ...
  let selectedTimeGrid: boolean[][] = Array(7).fill(null).map(() => Array(gridSections.length).fill(false));
  let campusFilterValue: string = '';
  ...
  function getCourseCampusForGroup(rows: HTMLTableRowElement[]): string[] {
    const campusSet: Set<string> = new Set<string>();
    rows.forEach(row => {
      const tds: NodeListOf<HTMLTableCellElement> = row.querySelectorAll('td');
      if (tds.length > 0) {
  ...
  function getCourseTeacherForGroup(rows: HTMLTableRowElement[]): string[] {
  ...
  function getCourseRestrictionForGroup(rows: HTMLTableRowElement[]): string {
    // Try to parse checkbox JSON value first
    try {
      const firstRow: HTMLTableRowElement = rows[0];
      const checkbox = firstRow.querySelector('input[type="checkbox"][name="kcId"]') as HTMLInputElement | null;
      if (checkbox?.value) {
        try {
          const data: any = JSON.parse(checkbox.value);
          const r: string = (data.xkxzsm || data.xkbz || '').trim();
          if (r) return r;
        } catch (e) {
          // ignore
        }
    ...
    for (const row of rows) {
      ...
      if (spanWithTitle) {
        const t: string = (spanWithTitle.getAttribute('title') || '').trim();
        if (t) return t;
      }
    }
    return '';
  ...
  function getAllRestrictionsCurrentPage(): void {
    allRestrictionsSet.clear();
    const groups: CourseGroup = groupRowsByCheckbox();
    Object.values(groups).forEach(rows => {
      const r: string = getCourseRestrictionForGroup(rows);
      if (!r) {
  ...
  function normalizeRestrictionText(s: string): string {
    if (!s) return '';
  ...
  function simplifyForCompare(s: string): string {
    if (!s) return '';
    const norm: string = normalizeRestrictionText(s).toLowerCase();
  ...
  function diceCoefficient(a: string, b: string): number {
    if (!a || !b) return 0;
    const A: string[] = getBigrams(a);
    const B: string[] = getBigrams(b);
  ...
  function isCourseRestrictionValid(restrictionStr: string): boolean {
    ...
    for (const rf of restrictionFilters) {
      ...
      const normRf: string = normalizeRestrictionText(rf);
      ...
      const simpRf: string = simplifyForCompare(normRf);
      if (simpRf && simpCourse) {
        const sim: number = diceCoefficient(simpCourse, simpRf);
  ...
  function isCourseTimeInGrid(courseTimeStr: string): boolean {
    ...
    while ((m = regex.exec(courseTimeStr)) !== null) {
      ...
      for (let sIdx = 0; sIdx < gridSections.length; sIdx++) {
        ...
        const contained: boolean = (sect.start <= sectionStart) && (sectionEnd <= sect.end);
  ...
  function isCourseCampusValid(campusArr: string[]): boolean {
  ...
  function filterCoursesByAllFilters(): void {
    ...
    Object.values(groups).forEach(rows => {
      ...
      const teacherArr: string[] = getCourseTeacherForGroup(rows);
      const restrictionStr: string = getCourseRestrictionForGroup(rows);

      const allTimeFit: boolean = times.every(isCourseTimeInGrid);
      const campusFit: boolean = isCourseCampusValid(campusArr);
      const teacherFit: boolean = isCourseTeacherValid(teacherArr);
  ...
  function createPanel(): void {
    ...
    // 事件绑定
    const timeBtn = document.getElementById('cf-time');
  ...
  function showCampusFilterModal(): void {
    ...
    if (okBtn) {
      okBtn.onclick = () => {
        const sel = document.getElementById('cf-campus-select') as HTMLSelectElement | null;
        if (sel) {
  ...
  function showTeacherFilterModal(): void {
    ...
    function renderList(): void {
      ...
      (listDiv.querySelectorAll('.cf-t-rem') || []).forEach(b => {
        b.addEventListener('click', () => {
          ...
          if (mode === 'white') {
            whiteCopy.splice(i, 1);
          } else {
            blackCopy.splice(i, 1);
          }
          renderList();
        });
      });
    ...
    if (addBtn) {
      addBtn.onclick = () => {
        const sel = document.getElementById('cf-teacher-select') as HTMLSelectElement | null;
        if (!sel || !sel.value) return;
        const v: string = sel.value;
        if (mode === 'white') {
          if (!whiteCopy.includes(v)) whiteCopy.push(v);
        } else {
          if (!blackCopy.includes(v)) blackCopy.push(v);
        }
        renderList();
      };
    }
  ...
  function showRestrictionFilterModal(): void {
    ...
    const display = document.getElementById('cf-restrict-select-display') as HTMLElement | null;
    let localRestrictions: string[] = [...restrictionFilters];
    ...
    if (display) {
      display.onclick = (ev: MouseEvent) => {
        ...
        const left: number = Math.max(8, rect.left);
        const width: number = Math.min(window.innerWidth - left - 8, Math.max(rect.width, 320));
        const spaceBelow: number = Math.max(0, window.innerHeight - rect.bottom - 20);
        const spaceAbove: number = Math.max(0, rect.top - 20);
